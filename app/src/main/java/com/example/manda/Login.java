...
public class Login extends KJActivity {                 //登录界面活动
    ...
    public void login() {                                              //登录按钮监听事件
        if (isUserNameAndPwdValid()) {
            ...
            kjh.jsonPost("http://10.0.2.2:8080/v1/user/login", params, new HttpCallBack() {
                @Override
                public void onSuccess(String t) {
                    ...
                    try {
                        //层层解析
                        int result=0;//结果
                        JSONObject retObj1 = new JSONObject(t);//将json字符串转换成JSONObject格式
                        String msg = retObj1.getString("msg");
                        String sessionId = retObj1.getString("sessionId");
                        if(msg.equals("登录成功")){
                            result=1;
                        }
                        isLogin(result,sessionId);
                    }catch (Exception e){
                    }
                }
        });
    ...
    public void isLogin(int result,String sessionId){
        SharedPreferences.Editor editor =login_sp.edit();
